...
/* Nice Snippet | Source: http://stackoverflow.com/questions/8032312/find-specific-text-in-multiple-txt-files-in-php
...
// Example Usage: Posts: search(path, post) 
// Example Usage: Pages: search(path, '')
function search($pathToSearch, $type){	
	// Variable scope fix
	global $count;
	global $Site;
	global $search_term;
	...
	foreach($posts_list as $post){
		...
		foreach(file($post_path) as $fli=>$fl){
			if(stripos($fl, $search_term)!==false){
				...
				echo '<b>Line ' . ($fli+1) . ':</b> ' . str_ireplace($search_term, "<span class='highlight'>$search_term</span>", $fl) . '</div>';
				$count++;
				} 
			}
	}
...
<input type='radio' name='filter' value='onlyPosts'>Search Posts 
<input type='radio' name='filter' value='onlyPages'>Search Pages<br>
...
if (isset($_GET['searchQuery']) && $_GET['searchQuery'] != ''){
	...
	switch ($filter) {
		case 'all':
			search($path_posts_dir, "post");
			search($path_pages_dir, "");
			break;
